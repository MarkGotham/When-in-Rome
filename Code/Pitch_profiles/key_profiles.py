...
def get_relevant_dist(source: dict = QuinnWhite,
    ...
    Clearly this function doesn't seek to cover all the variety in this sources, but
    ...
    elif source == QuinnWhite:  # special case of non-transposition equivalence
        dict_key = mode + str(tonic)
    elif source == PrinceSchumuckler:
        dict_key = 'all_beats_' + mode  # ossia 'downbeat', debatable
    elif source == TemperleyDeClerq:
        dict_key = 'harmony_' + mode
    elif source in [deClerqTemperley, Vuvan, VuvanHughes]:
